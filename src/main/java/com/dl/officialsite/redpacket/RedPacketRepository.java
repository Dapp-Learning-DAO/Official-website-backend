...
public interface RedPacketRepository extends JpaRepository<RedPacket, String>,  JpaSpecificationExecutor <RedPacket> {
    ...
    @Query(value = "select * from red_packet where claimed_address like ?1 and chain_id = ?2 order by createTime desc", nativeQuery = true)
    List<RedPacket> findByClaimedPacket(@Param("address") String address, @Param("chainId") String chainId);


    @Query(value = "select * from red_packet where address_list like ?1 and  claimed_address not like ?1 and  expire_time < UNIX_TIMESTAMP(current_timestamp()) and chain_id = ?2  order by createTime desc", nativeQuery = true)
    ...
    @Query(value = "select * from red_packet where address_list like ?1 and  claimed_address not like ?1 and status = ?2 and expire_time > UNIX_TIMESTAMP(current_timestamp()) and chain_id = ?3  order by createTime desc", nativeQuery = true)
