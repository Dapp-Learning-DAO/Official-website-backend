...
public class EcdsaRecoverUtil {
    ...
    public static String verifyMessage(String message, String signature) {
        return EcdsaRecoverUtil.recoverAddress(EcdsaRecoverUtil.hashMessage(message), signature);
    }

    public static String hashMessage(String message) {
        return Hash.sha3(
            Numeric.toHexStringNoPrefix(
                (EcdsaRecoverUtil.MESSAGE_PREFIX + message.length() + message).getBytes(StandardCharsets.UTF_8)));
    }

    public static String recoverAddress(String digest, String signature) {
        SignatureData signatureData = EcdsaRecoverUtil.getSignatureData(signature);
        int header = 0;
